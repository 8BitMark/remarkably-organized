...
{#if months.length}
	<div class="months">
			...
			<a href="#{getMonthLink(month)}" class="month">
				...
				<div class="days">
					...
					{#each new Array(month.end.getUTCDate()) as caldate, day}
						...
						{#if day === 0 || (startWeekOnSunday ? (caldate.getUTCDay() + day) %7 === 0 : (month.start.getUTCDay() + day) %7 === 1)}
       ...
       {@const weekNum = getWeek(weekDate, startWeekOnSunday).weekSinceYear}
							<a class="day" style:grid-column=1 href="#{month.start.getUTCFullYear()}-wk{weekNum}">{(month.start.getUTCDay() + day)}-{weekNum}</a>
